...
import my.bookstore.core.jalo.MechanicStore;
...
public abstract class GeneratedBookstoreCoreManager extends Extension
{
	...
	**/
	...
	public MechanicStore createMechanicStore(final SessionContext ctx, final Map attributeValues)
	{
		try
		{
			ComposedType type = getTenant().getJaloConnection().getTypeManager().getComposedType( BookstoreCoreConstants.TC.MECHANICSTORE );
			return (MechanicStore)type.newInstance( ctx, attributeValues );
		}
		catch( JaloGenericCreationException e)
		{
			final Throwable cause = e.getCause();
		...
		{
			throw new JaloSystemException( e ,"error creating MechanicStore : "+e.getMessage(), 0 );
		}
	}
	
	public MechanicStore createMechanicStore(final Map attributeValues)
	{
		return createMechanicStore( getSession().getSessionContext(), attributeValues );
	}
	...
	{
		return (String)item.getProperty( ctx, BookstoreCoreConstants.Attributes.Product.FEEDBACK);
	}
	...
	{
		item.setProperty(ctx, BookstoreCoreConstants.Attributes.Product.FEEDBACK,value);
	}
